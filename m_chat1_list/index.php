...
function DispData($mode,$sort,$word,$key,$page,$lid,$token)
{
	...
	if($item==0) {
		...
		while ($item = mysqli_fetch_assoc($rs)) {
			...
			if($TITLE == '') {
				$StrSQL="SELECT DAT_O2.O2_DVAL01 FROM DAT_O2 join DAT_IINE on DAT_O2.OID = DAT_IINE.OIDT where DAT_IINE.MID='".$_SESSION['MID']."' and DAT_IINE.MIDT='".$_GET['mid2']."';";
				$rs2=mysqli_query(ConnDB(),$StrSQL);
				$item2 = mysqli_fetch_assoc($rs2);
				$TITLE = $item2['O2_DVAL01'];
			}
