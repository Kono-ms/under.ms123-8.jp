...
function Main()
{
	...
	if(!isset($_POST['mode']) || $_POST['mode']===""){
		...
		$midt=mysqli_real_escape_string(ConnDB(),$_GET['midt']);
	} else {
		...
		$midt=mysqli_real_escape_string(ConnDB(),$_POST['midt']);
	}
	...
	switch ($mode){
		...
		case "save":
			...
			$FieldValue[1] = $_SESSION['MID'];
			$FieldValue[2] = $midt;
			$FieldValue[4] = date('Y/m/d H:i:s');

			SaveData($key);
			SendMail($_SESSION['MID'], $midt);
			break;
		case "back":
			RequestData($obj,$a,$b,$key,$mode);
			$mode="edit";
	...
	DispData($mode,$sort,$word,$key,$page,$lid,$midt);
...
//戻値 
//=========================================================================================================
function SendMail($mid, $midt)
{

	eval(globals());

	$maildata = GetMailTemplate('直接お問い合わせ(M1)');
	...
	$item_midt=mysqli_fetch_assoc($rs);
	$email_midt = $item_midt['EMAIL'];
	$MailBody=str_replace("[MIDT_NAME]",$item_mid['M2_DVAL01'],$MailBody);

	// 自分の情報
	$StrSQL="SELECT * FROM DAT_M1 where MID='".$mid."'";
	$rs=mysqli_query(ConnDB(),$StrSQL);
	$item_mid=mysqli_fetch_assoc($rs);
	$email_mid = $item_mid['EMAIL'];
	$MailBody=str_replace("[MID_NAME]",$item_mid['M1_DVAL01'],$MailBody);
	
	
	$MailBody=str_replace("[COMMENT]",$FieldValue[3],$MailBody);

	mb_language("Japanese");
	mb_internal_encoding("UTF-8");

	// 相手に送る
	$MailBody1=str_replace("[D-NAME]",$item_midt["M2_DVAL01"],$MailBody);
	mb_send_mail($email_midt, $subject, $MailBody1, "From:".mb_encode_mimeheader(mb_convert_encoding(SENDER_NAME,"ISO-2022-JP","AUTO"))."<".SENDER_EMAIL.">"); 
	// 自分に送る
	$MailBody2=str_replace("[D-NAME]",$item_mid["M1_DVAL01"],$MailBody);
	mb_send_mail($email_mid, $subject, $MailBody2, "From:".mb_encode_mimeheader(mb_convert_encoding(SENDER_NAME,"ISO-2022-JP","AUTO"))."<".SENDER_EMAIL.">"); 
}
...
//戻値 
//=========================================================================================================
function DispData($mode,$sort,$word,$key,$page,$lid,$midt)
{
  ...
  $htmllist = "./contact_list.html";
	...
	if ($mode!="list"){
		...
		$str=str_replace("[MIDT_NAME]",$item['M2_DVAL01'],$str);

		// フォーム
		$str=str_replace("[COMMENT]",$FieldValue[3],$str);
		$str=str_replace("[MIDT]",$midt,$str);
		...
		for ($i=0; $i<=$FieldMax; $i=$i+1){
			...
			if ($FieldAtt[$i]=="1"){
				...
				for ($j=0; $j<count($tmp); $j=$j+1) {
					$strtmp=$strtmp."<li><input type='radio' name='SYUBETSU' value='".$FieldName[$i].":".$tmp[$j]."' required><label>".$tmp[$j]."</label></li>";
				}
