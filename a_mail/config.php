...
//SELECT, RADIO, CHECKBOX時の値群
  $FieldParam[0]="";
  $FieldParam[1]="お問い合わせ::パスワード再送信(M1)::パスワード再送信(M2)::会員仮登録(M1)::会員仮登録(M2)::会員登録完了(M1)::会員登録完了(M2)::いいね(M1)::いいね(M2)::金額提示::提示の承認/拒否::評価依頼::新着マッチング::直接お問い合わせ(M1)::直接お問い合わせ(M2)::メッセージ通知（M1）::メッセージ通知（M2）::ステータス通知（M1）::ステータス通知（M2）::ステータス通知（M1）（メッセージ中）::ステータス通知（M2）（メッセージ中）";
  $FieldParam[2]="";
  $FieldParam[3]="";
  $FieldParam[4]="";
