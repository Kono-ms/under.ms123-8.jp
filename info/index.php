...
function Main()
{
	...
	if($mode=="disp"){
		...
		$StrSQL="SELECT * FROM DAT_INFO where ID='".$key."';";
		...
		header("Location: ".BASE_URL . "/info/".$url."/");
		exit();
	}
	...
	if ($word==""){
		...
		$StrSQL="SELECT * FROM DAT_INFO where URL='".$word."';";
		$rs=mysqli_query(ConnDB(),$StrSQL);
		$item=mysqli_fetch_assoc($rs);
		$url=$item['URL'];
		if($url!=""){
...
function DispData($mode,$sort,$word,$key,$page,$lid)
{
	...
	if ($mode!="list"){
		$filename="info_disp_default.html";
		...
		if($FieldValue[4]!=""){
			...
			$item=mysqli_fetch_assoc($rs);
			if($item['ETC03'] != '') {
				$filename = str_replace('ETC03:', '', $item['ETC03']);
			}
		}
		...
		else
	{
		...
		if($word!=""){
			$filename="info_list_default.html";
		} else {
		...
		if($word!=""){
			...
			if($word==""){
 			...
 			// yamamoto
			if($item['ETC02'] != '') {
				$filename = str_replace('ETC02:', '', $item['ETC02']);
			}
		}
		...
		$StrSQL="";
		// 公開先：公開用
		$StrSQL="SELECT DAT_INFO.* FROM ".$TableName." join DAT_CCATE on DAT_INFO.CCATE = concat('CCATE:', DAT_CCATE.CNAME) AND DAT_CCATE.ETC01 like '%ETC01:公開用%' ".ListSqlS($sort,$word)." LIMIT 0, 100;";
echo "<!--公開先:".$StrSQL."-->";
		$rs=mysqli_query(ConnDB(),$StrSQL);
		...
		if($item==0) {
			...
			for ($i=$s; $i<=$e; $i=$i+1) {
				if ($i==intval($page)) {
					...
					$str=$str."<li><a href=\"/info/?mode=list&lid=".$lid."&sort=".$sort."&word=".$url."&page=".$i."\">".$i."</a></li>";
				} 
			}
	...
	if($word==""){
		$tmp="";
		// 公開先：公開用
		$StrSQL="SELECT * FROM DAT_CCATE where CNAME<>'サイトオリジナル' AND ETC01 like '%ETC01:公開用%' order by ID";
		$rs=mysqli_query(ConnDB(),$StrSQL);
		$item=mysqli_num_rows($rs);
		while ($item = mysqli_fetch_assoc($rs)) {
...
//<li style=\"padding:10px 0px;border-bottom:0px;\"><div class=\"info_lst\"><p class=\"info_lst_fl\"><a href=\"/info/".$item['URL']."/\"><img src=\"".$item['CPIC']."\" alt=\"".$item['CNAME']."\" width=\"160\" /></a></p><p class=\"info_lst_fr\"><a href=\"/info/".$item['URL']."/\">".$item['CCOMMENT']."</a></p></div></li>";
		...
		$str=str_replace("[PAN]", "<li class=\"breadcrumb__item\" itemprop=\"itemListElement\" itemscope itemtype=\"http://schema.org/Listitem\"><a href=\"/info/\" itemprop=\"item\"><span itemprop=\"name\">記事コンテンツ</span></a><meta itemprop=\"position\" content=\"2\" />&nbsp;&gt;&nbsp;</li><li>".$word."<meta itemprop=\"position\" content=\"3\" /></li>", $str);
	...
	$tmp="";
	$StrSQL="SELECT DAT_INFO.* FROM DAT_INFO join DAT_CCATE on DAT_INFO.CCATE = concat('CCATE:', DAT_CCATE.CNAME) where DAT_INFO.ETC12='ETC12:公開中' AND DAT_CCATE.ETC01 like '%ETC01:公開用%' and DAT_INFO.CCATE like '".mb_substr($cate,0,10,"UTF-8")."%' order by DAT_INFO.DATE desc, DAT_INFO.ID desc LIMIT 0, 8;";
	$rs=mysqli_query(ConnDB(),$StrSQL);
	$item=mysqli_num_rows($rs);
	while ($item = mysqli_fetch_assoc($rs)) {
		$tmp.="<a href=\"/info/[URL]/\" class=\"similar_column\"><div><div class=\"similar_column_img\"> <img src=\"[PIC]\" alt=\"\"> </div><div class=\"similar_column_TITLE\"><p>[D-TITLE]</p><p>[COMMENT]</p></div><div class=\"similar_column_detail\"><p>[COMMENT]</p></div></div></a> ";
		...
		$tmp=str_replace("[URL]",$item['URL'],$tmp);
	} 
	...
	$cnt=1;
	$tmp="";
	$StrSQL="SELECT DAT_INFO.* FROM DAT_INFO join DAT_CCATE on DAT_INFO.CCATE = concat('CCATE:', DAT_CCATE.CNAME) where DAT_INFO.ETC12='ETC12:公開中' AND DAT_CCATE.ETC01 like '%ETC01:公開用%' and DAT_INFO.CCATE<>'CCATE:特集' order by DAT_INFO.ETC12, DATE desc, DAT_INFO.ID desc LIMIT 0, 10;";
	$rs=mysqli_query(ConnDB(),$StrSQL);
	$item=mysqli_num_rows($rs);
	while ($item = mysqli_fetch_assoc($rs)) {
		$tmp.="<a href=\"/info/[URL]/\" class=\"ranking_column\"><div><div class=\"ranking_column_img\"> <img src=\"[PIC]\" alt=\"\"> <img src=\"/common/images/side_rank_[CNT].png\" alt=\"\"> </div><div class=\"ranking_column_TITLE\"> <img src=\"/common/images/side_rank_[CNT].png\" alt=\"\"><p>[D-TITLE]</p><p>[COMMENT]</p></div></div></a> ";
		$tmp=str_replace("[D-DATE]",$item['DATE'],$tmp);
		$tmp=str_replace("[D-CCATE]",str_replace("CCATE:", "", $item['CCATE']),$tmp);
		$tmp=str_replace("[PIC]",$item['PIC'],$tmp);
		$tmp=str_replace("[D-TITLE]",$item['TITLE'],$tmp);
	...
	$cnt=1;
	$tmp="";
	$StrSQL="SELECT DAT_INFO.* FROM DAT_INFO join DAT_CCATE on DAT_INFO.CCATE = concat('CCATE:', DAT_CCATE.CNAME) where DAT_INFO.ETC12='ETC12:公開中' AND DAT_CCATE.ETC01 like '%ETC01:公開用%' and DAT_INFO.CCATE<>'CCATE:特集' and DAT_INFO.ETC12='ETC12:公開中' order by DAT_INFO.ETC12, DATE desc, DAT_INFO.ID desc LIMIT 0, 10;";
	$rs=mysqli_query(ConnDB(),$StrSQL);
	$item=mysqli_num_rows($rs);
	while ($item = mysqli_fetch_assoc($rs)) {
		$tmp.="<li><a href=\"/info/[URL]/\" class=\"ranking_column\"><div><div class=\"ranking_column_img\"> <img src=\"[PIC]\" alt=\"\"> <img src=\"/common/images/side_rank_[CNT].png\" alt=\"\"> </div><div class=\"ranking_column_TITLE\"> <img src=\"/common/images/side_rank_[CNT].png\" alt=\"\"><p>[D-TITLE]</p><p>[COMMENT]</p></div></div></a></li> ";
		$tmp=str_replace("[D-DATE]",$item['DATE'],$tmp);
		$tmp=str_replace("[D-CCATE]",str_replace("CCATE:", "", $item['CCATE']),$tmp);
		$tmp=str_replace("[PIC]",$item['PIC'],$tmp);
		$tmp=str_replace("[D-TITLE]",$item['TITLE'],$tmp);
