...
function Main()
{
	...
	if($TITLE == '') {
		$StrSQL="SELECT DAT_O2.O2_DVAL01 FROM DAT_O2 join DAT_IINE on DAT_O2.OID = DAT_IINE.OIDT where DAT_IINE.MID='".$mid1."' and DAT_IINE.MIDT='".$mid2."';";
		$rs=mysqli_query(ConnDB(),$StrSQL);
		$item = mysqli_fetch_assoc($rs);
		$TITLE = $item['O2_DVAL01'];
	}
...
function SendMailMsg($mid1,$mid2)
{
	...
	$maildata = GetMailTemplate('メッセージ通知（M2）');
...
function DispData($mode,$sort,$word,$key,$page,$lid,$token,$mid1,$mid2)
{
	...
	$StrSQL="SELECT CASE WHEN ifnull(EDITDATE,'')='' THEN NEWDATE ELSE EDITDATE END as NEWDATE  FROM DAT_MCONTACT where MID='".$_SESSION['MID']."' and MIDT='".$mid2."' and ETC02='".$_GET['etc02']."' and ifnull(ETC03,'')='".$_GET['etc03']."' order by CASE WHEN ifnull(EDITDATE,'')='' THEN NEWDATE ELSE EDITDATE END desc;";
	$rs=mysqli_query(ConnDB(),$StrSQL);
	$item = mysqli_fetch_assoc($rs);
	$str=str_replace("[NEWDATE]",$item["NEWDATE"],$str);
	
	//最新ステータス取得
	$StrSQL="SELECT * FROM DAT_MCONTACT WHERE MID = '".$_SESSION['MID']."' AND MIDT = '".$mid2."' AND ETC02 = '".$_GET['etc02']."' and ifnull(ETC03,'')='".$_GET['etc03']."' order by id desc";
	$rs=mysqli_query(ConnDB(),$StrSQL);
	$item = mysqli_fetch_assoc($rs); 
