...
function DispData($mode,$sort,$word,$key,$page,$lid,$token)
{
	...
	$uid=$_GET['uid'];
	$StrSQL="SELECT *, NEWDATE as LKDATE, ID as CID FROM DAT_MESSAGE where AID='".$word."' and ETC02='".$_GET['etc02']."' and ifnull(ETC03,'')='".$_GET['etc03']."' and DAT_MESSAGE.ENABLE='ENABLE:公開中' order by DAT_MESSAGE.ID desc LIMIT 0, 1;";
	$rs=mysqli_query(ConnDB(),$StrSQL);
	$item=mysqli_num_rows($rs);
	if($item>0) {
		$item = mysqli_fetch_assoc($rs);

		if($_SESSION["CHATID".$uid]<$item['CID']){
			...
			if(substr($item['COMMENT'], 0, 1) == '[') {
				...
				$tmp="<div class=\"right_balloon\"><div class=\"faceicon\"><img src=\"" . $img_m1 . "\"/></div><div class=\"balloon_div_right\"><div class=\"balloon_sub\"><span class=\"balloon_date\">[LKDATE]</span><span class=\"balloon_kidoku\">[KIDOKU]</span></div><p>[D-COMMENT]</p></div></div>";
				...
				$tmp="<div class=\"left_balloon\"><div class=\"faceicon\"><img src=\"" . $img_m2 . "\"/></div><div class=\"balloon_div_right\"><div class=\"balloon_sub\"><span class=\"balloon_name\">[LKNAME]</span><span class=\"balloon_date\">[LKDATE]</span><span class=\"balloon_kidoku\">[KIDOKU]</span></div><p>[D-COMMENT]</p></div></div>";
