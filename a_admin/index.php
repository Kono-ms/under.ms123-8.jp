...
//各テンプレートファイル名
  ...
  $htmllist="list.html";
	...
	if ($mode!="list"){
		...
		if($errmsg<>""){
			$str=str_replace("[ERR_MSG]",$errmsg,$str);
...
function SaveData($key)
{
	...
	if($item==0) {
		...
		$StrSQL=$StrSQL.") VALUES (";
		for ($i=1; $i<=$FieldMax-1; $i=$i+1) {
			$StrSQL=$StrSQL."'".mysqli_real_escape_string(ConnDB(),htmlspecialchars($FieldValue[$i]))."',";
		}
		...
		$StrSQL="UPDATE ".$TableName." SET ";
		for ($i=1; $i<=$FieldMax-1; $i=$i+1) {
			$StrSQL=$StrSQL."`".$FieldName[$i]."`='".mysqli_real_escape_string(ConnDB(),htmlspecialchars($FieldValue[$i]))."',";
		}
