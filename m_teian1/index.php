...
function Main()
{
	...
	if ($mode=="teian"){
		...
		$StrSQL="SELECT OID from DAT_O1 where MID='".$_SESSION['MID']."' order by cast(replace(OID,concat('".$_SESSION['MID']."','-'),'') as signed) desc limit 0,1;";
		$rs=mysqli_query(ConnDB(),$StrSQL);
		$item = mysqli_fetch_assoc($rs);
		$lastNum = ($item && isset($item['OID'])) ? intval(str_replace($_SESSION['MID']."-", "", $item['OID'])) : 0;
		$oid=$_SESSION['MID']."-".sprintf("%02d", $lastNum+1);


		$StrSQL="INSERT INTO DAT_O1 (OID, MID, ENABLE,O1_DFIL01,O1_DFIL02,O1_DFIL03,O1_DFIL04,O1_DFIL05,O1_DFIL06,O1_DFIL07,O1_DFIL08,O1_DFIL09,O1_DFIL10,NEWDATE, EDITDATE) values (";
		$StrSQL.="'".$oid."',";
		$StrSQL.="'".$mid1."',";
		$StrSQL.="'ENABLE:公開中',";
		$StrSQL.="'/a_o1/data/s.gif',";
		$StrSQL.="'/a_o1/data/s.gif',";
		$StrSQL.="'/a_o1/data/s.gif',";
		$StrSQL.="'/a_o1/data/s.gif',";
		$StrSQL.="'/a_o1/data/s.gif',";
		...
		$StrSQL.="'".date("Y/m/d H:i:s")."'";
		$StrSQL.=")";
		echo "<!--INSERT INTO DAT_O1:".$StrSQL."-->";
		...
		$StrSQL="SELECT ID,OID FROM DAT_O1 where MID='".$_SESSION['MID']."' AND OID = '".$oid."'  order by ID;";
		$rs=mysqli_query(ConnDB(),$StrSQL);
		$item = mysqli_fetch_assoc($rs);
		$O1_id=$item["ID"];
		$key=$item["ID"];
		...
		for ($j=0; $j<count($sel); $j=$j+1) {
			...
			$StrSQL.="'ENABLE:公開中',";
			$StrSQL.="'".date("Y/m/d H:i:s")."',";
			$StrSQL.="'[" . str_replace('STATUS:', '', $status) . "]が送信されました。',";
			$StrSQL.="'".$next_id."',";
			$StrSQL.="'".$oid."',";
			$StrSQL.="'ETC10:オウカガイ'";
			$StrSQL.=")";

			echo "<!--INSERT INTO DAT_MESSAGE:".$StrSQL."-->";
			if (!(mysqli_query(ConnDB(),$StrSQL))) {
				var_dump($StrSQL);
				die;
			}
