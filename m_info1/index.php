...
echo "<!--filename:".$filename."-->";
		...
		$StrSQL="";
		// 公開先：不動産会社用
		$StrSQL="SELECT DAT_INFO.* FROM ".$TableName." join DAT_CCATE on DAT_INFO.CCATE = concat('CCATE:', DAT_CCATE.CNAME) AND DAT_CCATE.ETC01 like '%ETC01:不動産会社用%' ".ListSqlS($sort,$word)." LIMIT 0, 100;";
echo "<!--".$StrSQL."-->";
		$rs=mysqli_query(ConnDB(),$StrSQL);
	...
	if($word==""){
		$tmp="";
		// 公開先：不動産会社用
		$StrSQL="SELECT * FROM DAT_CCATE where CNAME<>'サイトオリジナル' AND ETC01 like '%ETC01:不動産会社用%' order by ID";
		$rs=mysqli_query(ConnDB(),$StrSQL);
		$item=mysqli_num_rows($rs);
		while ($item = mysqli_fetch_assoc($rs)) {
...
//<li style=\"padding:10px 0px;border-bottom:0px;\"><div class=\"info_lst\"><p class=\"info_lst_fl\"><a href=\"/info/".$item['URL']."/\"><img src=\"".$item['CPIC']."\" alt=\"".$item['CNAME']."\" width=\"160\" /></a></p><p class=\"info_lst_fr\"><a href=\"/info/".$item['URL']."/\">".$item['CCOMMENT']."</a></p></div></li>";
	...
	$tmp="";
	$StrSQL="SELECT DAT_INFO.* FROM DAT_INFO join DAT_CCATE on DAT_INFO.CCATE = concat('CCATE:', DAT_CCATE.CNAME) where DAT_INFO.ETC12='ETC12:公開中' AND DAT_CCATE.ETC01 like '%ETC01:不動産会社用%' and DAT_INFO.CCATE like '".mb_substr($cate,0,10,"UTF-8")."%' order by DAT_INFO.DATE desc, DAT_INFO.ID desc LIMIT 0, 8;";
	$rs=mysqli_query(ConnDB(),$StrSQL);
	$item=mysqli_num_rows($rs);
	while ($item = mysqli_fetch_assoc($rs)) {
	...
	$cnt=1;
	$tmp="";
	$StrSQL="SELECT DAT_INFO.* FROM DAT_INFO join DAT_CCATE on DAT_INFO.CCATE = concat('CCATE:', DAT_CCATE.CNAME) where DAT_INFO.ETC12='ETC12:公開中' AND DAT_CCATE.ETC01 like '%ETC01:不動産会社用%' and DAT_INFO.CCATE<>'CCATE:特集' order by DAT_INFO.ETC12, DATE desc, DAT_INFO.ID desc LIMIT 0, 10;";
	$rs=mysqli_query(ConnDB(),$StrSQL);
	$item=mysqli_num_rows($rs);
	while ($item = mysqli_fetch_assoc($rs)) {
	...
	$cnt=1;
	$tmp="";
	$StrSQL="SELECT DAT_INFO.* FROM DAT_INFO join DAT_CCATE on DAT_INFO.CCATE = concat('CCATE:', DAT_CCATE.CNAME) where DAT_INFO.ETC12='ETC12:公開中' AND DAT_CCATE.ETC01 like '%ETC01:不動産会社用%' and DAT_INFO.CCATE<>'CCATE:特集' and DAT_INFO.ETC12='ETC12:公開中' order by DAT_INFO.ETC12, DATE desc, DAT_INFO.ID desc LIMIT 0, 10;";
	$rs=mysqli_query(ConnDB(),$StrSQL);
	$item=mysqli_num_rows($rs);
	while ($item = mysqli_fetch_assoc($rs)) {
